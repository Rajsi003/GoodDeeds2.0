...
//     if (!newUser) throw new Error('Failed to save user to the database');
...
//   } catch (error) {
//     console.error("Error signing in user:", error);  // Log the full error
//     return null;
//   }
// }
...
//     if (!currentUser || currentUser.documents.length === 0) throw new Error('User data not found');

//     return currentUser.documents[0];  // Return the user document
//   } catch (error) {
...
import { Client, Storage, ImageGravity } from "appwrite";
...
export async function createPost(post: INewPost) {
  try {
    ...
    // Fetch the user's current post count
    const userPostsCount = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.postCollectionId,
    ...
    const updatedUser = await databases.updateDocument(
      ...
      userId,
      {
        score: (userPostsCount.total).toString(), // Set the score to the total number of posts
      }
    );
...
export async function searchPosts(searchTerm: string) {
  try {
    const posts = await databases.listDocuments(
      ...
      appwriteConfig.postCollectionId,
      [
        // Search for caption using Query.search
